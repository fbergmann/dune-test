    ...
    public:
      ...
      void set(N & newton) const
      {
        if (this->hasKey("ReassembleThreshold"))
          newton.setReassembleThreshold(this->get<double>("ReassembleThreshold"));

        if (this->hasKey("LineSearchMaxIterations"))
          newton.setLineSearchMaxIterations(this->get<int>("LineSearchMaxIterations"));

        if (this->hasKey("MaxIterations"))
          newton.setMaxIterations(this->get<int>("MaxIterations"));

        if (this->hasKey("AbsoluteLimit"))
          newton.setAbsoluteLimit(this->get<double>("AbsoluteLimit"));

        if (this->hasKey("Reduction"))
          newton.setReduction(this->get<double>("Reduction"));

        if (this->hasKey("LinearReduction"))
          newton.setMinLinearReduction(this->get<double>("LinearReduction"));

        if (this->hasKey("LineSearchDamping"))
          newton.setLineSearchDampingFactor(this->get<double>("LineSearchDamping"));

        if (this->hasKey("Verbosity"))
          newton.setVerbosityLevel(this->get<int>("Verbosity"));
      }
    };
