...
#include"solutiontimeoutput.hh"
#include"timemanager.hh"
#include"newtonutilities.hh"
#include"pvdwriter.hh"
...
void run (const GV& gv, Dune::ParameterTree & param)
{
    ...
    //// typedef Dune::PDELab::ISTLVectorBackend<Dune::PDELab::ISTLParameters::static_blocking,3> VBE; //new : the components number
    ...
    typedef Dune::PVDWriter<GV,VTKWriter> PVDWriter;
    PVDWriter pvdwriter(gv,param.get<int>("subsampling",1),basename);
    ...
    typedef GnuplotSolutionTimeOutput<RF,GV> GnuplotSolutionTimeOutput;
    GnuplotSolutionTimeOutput solutiontimeoutput("output.dat",gv);
*/
    // <<<9>>> time loop
    U unew(gfs,0.0);
    unew = uold;
