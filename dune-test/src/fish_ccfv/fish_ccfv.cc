...
#include "../turing_problem_ccfv/ccfv_local_operator.hh"
    ...
    for (int i = 1; i < argc; i++)
      {
        if (std::string(argv[i]) == "--help" || std::string(argv[i]) == "-h")
          {
            if(helper.rank()==0)
              std::cout << "usage: ./fish_ccfv <configfile> [OPTIONS]" << std::endl;
            return 0;
          }
      }
        ...
        for(Iterator it = mdgv.template begin<0>(); it != mdgv.template end<0>(); ++it)
          {
            ...
            if (isInside(center) && it->partitionType()==Dune::InteriorEntity)
              {
